...
pub enum Command {
    /// Submit a new Rune Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Rune Cloud task.
    Status(StatusCommand),
    /// List Rune Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Rune Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Rune Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Rune Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `rune cloud` to browse).
    ...
    /// Git branch to run in Rune Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Rune Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Rune Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Rune Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
