...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = rune_command(rune_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(rune_home.path()).await?;
    ...
    let mut list_cmd = rune_command(rune_home.path())?;
    ...
    let mut list_json_cmd = rune_command(rune_home.path())?;
    ...
    let mut get_cmd = rune_command(rune_home.path())?;
    ...
    assert!(stdout.contains("remove: rune mcp remove docs"));

    let mut get_json_cmd = rune_command(rune_home.path())?;
    get_json_cmd
