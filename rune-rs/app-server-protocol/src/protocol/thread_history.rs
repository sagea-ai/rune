...
use rune_protocol::protocol::AgentReasoningEvent;
use rune_protocol::protocol::AgentReasoningRawContentEvent;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `rune-rs/core/rollout/policy.rs`.
    ...
    fn handle_item_completed(&mut self, payload: &ItemCompletedEvent) {
        if let rune_protocol::items::TurnItem::Plan(plan) = &payload.item {
