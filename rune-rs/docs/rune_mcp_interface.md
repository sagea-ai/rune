# Rune MCP Server Interface [experimental]

This document describes Rune’s experimental MCP server interface: a JSON‑RPC API that runs over the Model Context Protocol (MCP) transport to control a local Rune engine.

- Status: experimental and subject to change without notice
- Server binary: `rune mcp-server` (or `rune-mcp-server`)
- Transport: standard MCP over stdio (JSON‑RPC 2.0, line‑delimited)

## Overview

Rune exposes a small set of MCP‑compatible methods to create and manage conversations, send user input, receive live events, and handle approval prompts. The types are defined in `protocol/src/mcp_protocol.rs` and re‑used by the MCP server implementation in `mcp-server/`.

At a glance:

- Conversations
  - `newConversation` → start a Rune session
...
- Notifications (server → client)
  ...
  - `rune/event` stream with agent events

See code for full type definitions and exact shapes: `protocol/src/mcp_protocol.rs`.

## Starting the server

Run Rune as an MCP server and connect an MCP client:

```bash
rune mcp-server | your_mcp_client
```

For a simple inspection UI, you can also try:

```bash
npx @modelcontextprotocol/inspector rune mcp-server
```

Use the separate `rune mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
- `model`: string model id (e.g. "o3", "gpt-5.1", "gpt-5.1-rune")
...
Fetch the catalog of models available in the current Rune build with `model/list`. The request accepts optional pagination inputs:
...
- `rune/event` with the serialized Rune event payload. The shape matches `core/src/protocol.rs`’s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
The `rune` and `rune-reply` tools return standard MCP `CallToolResult` payloads. For
compatibility with MCP clients that prefer `structuredContent`, Rune mirrors the
...
```json
{
  "content": [{ "type": "text", "text": "Hello from Rune" }],
  "structuredContent": {
    "threadId": "019bbed6-1e9e-7f31-984c-a05b65045719",
    "content": "Hello from Rune"
  }
}
```
...
When Rune needs approval to apply changes or run commands, the server issues JSON‑RPC requests to the client:
...
```json
{ "jsonrpc": "2.0", "id": 2, "method": "sendUserMessage", "params": { "conversationId": "c7b0…", "items": [{ "type": "text", "text": "Hello Rune" }] } }
```

While processing, the server emits `rune/event` notifications containing agent output, approvals, and status updates.
