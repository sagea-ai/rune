...
impl TestRuneBuilder {
    ...
    pub fn with_auth(mut self, auth: RuneAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestRune> {
    ...
    async fn build_from_config(
        ...
        let new_conversation = match resume_from {
            Some(path) => {
                let auth_manager = rune_core::AuthManager::from_auth_for_testing(auth);
                thread_manager
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = rune_utils_cargo_bin::cargo_bin("rune") {
            config.rune_linux_sandbox_exe = Some(path);
        }
...
impl TestRune {
    ...
    pub fn rune_home_path(&self) -> &Path {
    ...
    pub async fn submit_turn_with_policies(
        ...
        wait_for_event(&self.rune, |event| {
...
pub struct TestRuneHarness {
    server: MockServer,
    test: TestRune,
}

impl TestRuneHarness {
    pub async fn new() -> Result<Self> {
        Self::with_builder(test_rune()).await
    }

    pub async fn with_config(mutator: impl FnOnce(&mut Config) + Send + 'static) -> Result<Self> {
        Self::with_builder(test_rune().with_config(mutator)).await
    }

    pub async fn with_builder(mut builder: TestRuneBuilder) -> Result<Self> {
...
pub fn test_rune() -> TestRuneBuilder {
    TestRuneBuilder {
        config_mutators: vec![],
        auth: RuneAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
        home: None,
    }
}
