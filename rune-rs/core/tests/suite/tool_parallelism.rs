...
async fn run_turn(test: &TestRune, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestRune, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_rune_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestRune> {
    let mut builder = test_rune().with_model("test-gpt-5.1-rune");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_rune_with_test_tool(&server).await?;
