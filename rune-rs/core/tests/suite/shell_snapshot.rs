...
async fn wait_for_snapshot(rune_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = rune_home.join("shell_snapshots");
...
async fn run_snapshot_command(command: &str) -> Result<SnapshotRun> {
    ...
    let rune_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&rune_home).await?;
    let snapshot_content = fs::read_to_string(&snapshot_path).await?;

    let end = wait_for_event_match(&rune, |ev| match ev {
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.rune_home));
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let rune_home = home.path().to_path_buf();
