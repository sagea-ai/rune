...
use rune_utils_cargo_bin::cargo_bin;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.rune, |ev| {
    ...
    let end_event = wait_for_event(&fixture.rune, |ev| {
    ...
    wait_for_event(&fixture.rune, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
/// This test writes to a fallback credentials file in RUNE_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire RUNE_HOME through the code.
#[serial(rune_home)]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn streamable_http_with_oauth_round_trip() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("RUNE_HOME", temp_home.path().as_os_str());
