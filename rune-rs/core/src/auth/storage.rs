...
/// Determine where Rune should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in RUNE_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in RUNE_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Expected structure for $RUNE_HOME/auth.json.
...
pub(super) fn get_auth_file(rune_home: &Path) -> PathBuf {
    rune_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(rune_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(rune_home: PathBuf) -> Self {
        Self { rune_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `RUNE_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.rune_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.rune_home)
    }
}

const KEYRING_SERVICE: &str = "Rune Auth";

// turns rune_home path into a stable, short key string
fn compute_store_key(rune_home: &Path) -> std::io::Result<String> {
    let canonical = rune_home
        .canonicalize()
        .unwrap_or_else(|_| rune_home.to_path_buf());
...
impl KeyringAuthStorage {
    fn new(rune_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            rune_home,
            keyring_store,
        }
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.rune_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.rune_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.rune_home)?;
...
impl AutoAuthStorage {
    fn new(rune_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            keyring_storage: Arc::new(KeyringAuthStorage::new(rune_home.clone(), keyring_store)),
            file_storage: Arc::new(FileAuthStorage::new(rune_home)),
        }
    }
}
...
// A global in-memory store for mapping rune_home -> AuthDotJson.
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_keyring_store(rune_home, mode, keyring_store)
}
...
fn create_auth_storage_with_keyring_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(rune_home)),
        AuthCredentialsStoreMode::Keyring => {
            Arc::new(KeyringAuthStorage::new(rune_home, keyring_store))
        }
        AuthCredentialsStoreMode::Auto => Arc::new(AutoAuthStorage::new(rune_home, keyring_store)),
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(rune_home)),
    }
}
...
mod tests {
    ...
    async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
        ...
        let storage = FileAuthStorage::new(rune_home.path().to_path_buf());
    ...
    async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
        ...
        let file = get_auth_file(rune_home.path());
        storage
            .save(&auth_dot_json)
            .context("failed to save auth file")?;
    ...
    fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
        ...
        seed_keyring_with_auth(
            &mock_keyring,
            || compute_store_key(rune_home.path()),
            &expected,
        )?;
    ...
    fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
        let rune_home = PathBuf::from("~/.rune");

        let key = compute_store_key(rune_home.as_path())?;
    ...
    fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
        ...
        let auth_file = get_auth_file(rune_home.path());
        std::fs::write(&auth_file, "stale")?;
        ...
        let key = compute_store_key(rune_home.path())?;
    ...
    fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
        let rune_home = tempdir()?;
        let mock_keyring = MockKeyringStore::default();
        let storage = AutoAuthStorage::new(rune_home.path().to_path_buf(), Arc::new(mock_keyring));
