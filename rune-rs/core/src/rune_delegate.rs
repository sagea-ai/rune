...
use rune_protocol::protocol::ApplyPatchApprovalRequestEvent;
use rune_protocol::protocol::Event;
use rune_protocol::protocol::EventMsg;
use rune_protocol::protocol::ExecApprovalRequestEvent;
use rune_protocol::protocol::Op;
use rune_protocol::protocol::RequestUserInputEvent;
use rune_protocol::protocol::SessionSource;
use rune_protocol::protocol::SubAgentSource;
use rune_protocol::protocol::Submission;
...
use crate::rune::SUBMISSION_CHANNEL_CAPACITY;
...
/// Start an interactive sub-Rune thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
pub(crate) async fn run_rune_thread_interactive(
    config: Config,
    ...
    let RuneSpawnOk { rune, .. } = Rune::spawn(
        config,
        auth_manager,
        models_manager,
    ...
    let rune_for_events = Arc::clone(&rune);
    ...
    let rune_for_ops = Arc::clone(&rune);
    tokio::spawn(async move {
        forward_ops(rune_for_ops, rx_ops, cancel_token_ops).await;
    });
    ...
    Ok(Rune {
        ...
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: rune.agent_status.clone(),
        session: Arc::clone(&rune.session),
    })
}
...
pub(crate) async fn run_rune_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: Arc<ModelsManager>,
    input: Vec<UserInput>,
    ...
    let io = run_rune_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&rune).await;
                break;
            }
            event = rune.next_event() => {
...
async fn shutdown_delegate(rune: &Rune) {
    let _ = rune.submit(Op::Interrupt).await;
    let _ = rune.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = rune.next_event().await {
            if matches!(
                event.msg,
...
async fn handle_exec_approval(
    ...
    let _ = rune.submit(Op::ExecApproval { id, decision }).await;
}
...
async fn handle_patch_approval(
    ...
    .await;
    let _ = rune.submit(Op::PatchApproval { id, decision }).await;
}
...
async fn handle_request_user_input(
    ...
    .await;
    let _ = rune.submit(Op::UserInputAnswer { id, response }).await;
}
...
async fn await_approval_with_cancel<F>(
    ...
    sub_id: &str,
    cancel_token: &CancellationToken,
) -> rune_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = rune_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            parent_session
                .notify_approval(sub_id, rune_protocol::protocol::ReviewDecision::Abort)
                .await;
            rune_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
...
mod tests {
    ...
    async fn forward_events_cancelled_while_send_blocked_shuts_down_delegate() {
        ...
        let (session, ctx, _rx_evt) = crate::rune::make_session_and_context_with_rx().await;
        let rune = Arc::new(Rune {
            next_id: AtomicU64::new(0),
            tx_sub,
            rx_event: rx_events,
