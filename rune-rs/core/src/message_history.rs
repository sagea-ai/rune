...
//!
//! The history is stored at `~/.rune/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.rune`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.rune_home.clone();
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.rune/history.jsonl` and ensure the parent directory exists.
...
mod tests {
    ...
    async fn append_entry_trims_history_when_beyond_max_bytes() {
        ...
        let history_path = rune_home.path().join("history.jsonl");
