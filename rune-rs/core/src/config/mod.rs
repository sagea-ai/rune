...
pub const CONFIG_TOML_FILE: &str = "rune.toml";
...
pub struct Config {
    ...
    /// True if the user passed in an override or set a value in rune.toml
    ...
    ///
    /// This is the same `tui.alternate_screen` value from `rune.toml` (see [`Tui`]).
