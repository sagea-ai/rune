...
use rune_protocol::dynamic_tools::DynamicToolResponse;
use rune_protocol::dynamic_tools::DynamicToolSpec;
use rune_protocol::items::PlanItem;
...
use rune_protocol::models::format_allow_prefixes;
use rune_protocol::openai_models::ModelInfo;
use rune_protocol::protocol::FileChange;
use rune_protocol::protocol::HasLegacyEvent;
use rune_protocol::protocol::ItemCompletedEvent;
use rune_protocol::protocol::ItemStartedEvent;
use rune_protocol::protocol::RawResponseItemEvent;
...
use crate::rune_thread::ThreadConfigSnapshot;
...
use rune_protocol::models::DeveloperInstructions;
...
use rune_utils_readiness::ReadinessFlag;
use tokio::sync::watch;

/// The high-level interface to the Rune system.
...
/// Wrapper returned by [`Rune::spawn`] containing the spawned [`Rune`],
/// the submission id for the initial `ConfigureSession` request and the
/// unique session id.
pub struct RuneSpawnOk {
    pub rune: Rune,
...
impl Rune {
    /// Spawn a new [`Rune`] and initialize the session.
    ...
    pub(crate) async fn spawn(
        ...
        let exec_policy = ExecPolicyManager::load(&config.config_layer_stack)
            .await
            .map_err(|err| RuneErr::Fatal(format!("failed to load rules: {err}")))?;
            ...
            match thread_id {
                Some(thread_id) => {
                    let state_db_ctx = state_db::open_if_present(
                        config.rune_home.as_path(),
                        config.model_provider_id.as_str(),
                    )
                    .await;
                    state_db::get_dynamic_tools(state_db_ctx.as_deref(), thread_id, "rune_spawn")
                        .await
                }
                None => None,
            }
        ...
        let session_configuration = SessionConfiguration {
            ...
            rune_home: config.rune_home.clone(),
            thread_name: None,
        ...
        // Generate a unique ID for the lifetime of this Rune session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.rune_home)
        })?;
    ...
    /// Use sparingly: prefer `submit()` so Rune is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, sub: Submission) -> RuneResult<()> {
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| RuneErr::InternalAgentDied)?;
        Ok(())
    }
...
pub(crate) struct SessionConfiguration {
    ...
    cwd: PathBuf,
    /// Directory containing all Rune state for this session.
    rune_home: PathBuf,
...
impl SessionConfiguration {
    pub(crate) fn rune_home(&self) -> &PathBuf {
        &self.rune_home
    }
...
impl Session {
    ...
    pub(crate) async fn rune_home(&self) -> PathBuf {
        let state = self.state.lock().await;
        state.session_configuration.rune_home().clone()
    }
    ...
    fn make_turn_context(
        ...
        TurnContext {
            ...
            rune_linux_sandbox_exe: per_turn_config.rune_linux_sandbox_exe.clone(),
    ...
    async fn new(
        ...
        let auth_mode = auth.map(RuneAuth::auth_mode).map(TelemetryAuthMode::from);
        let otel_manager = OtelManager::new(
            conversation_id,
            session_configuration.collaboration_mode.model(),
            session_configuration.collaboration_mode.model(),
            auth.and_then(RuneAuth::get_account_id),
            auth.and_then(RuneAuth::get_account_email),
            auth_mode,
        ...
        let thread_name =
            match session_index::find_thread_name_by_id(&config.rune_home, &conversation_id).await
            {
                Ok(name) => name,
                Err(err) => {
                ...
                if let Some(prev) = Self::last_model_name(&rollout_items, curr) {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Rune performance."
                            ),
                        }),
                    )
                    .await;
                ...
                Err(err) => {
                    ...
                    self.send_event_raw(Event {
                        ...
                        msg: EventMsg::Error(ErrorEvent {
                            ...
                            rune_error_info: Some(RuneErrorInfo::BadRequest),
                        }),
                    })
                    .await;
                    return Err(err);
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&rune_home, amendment)
            .await?;
    ...
    pub(crate) async fn notify_stream_error(
        ...
        let additional_details = rune_error.to_string();
        let rune_error_info = RuneErrorInfo::ResponseStreamDisconnected {
            http_status_code: rune_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            rune_error_info: Some(rune_error_info),
    ...
    async fn refresh_mcp_servers_inner(
        ...
        let mcp_servers = with_rune_apps_mcp(
            mcp_servers,
        ...
        let sandbox_state = SandboxState {
            ...
            rune_linux_sandbox_exe: turn_context.rune_linux_sandbox_exe.clone(),
...
mod handlers {
    ...
    use crate::rune::SessionSettingsUpdate;
    use crate::rune::SteerInputError;
    use crate::rune::TurnContext;

    use crate::rune::spawn_review_thread;
    use crate::config::Config;
    ...
    use rune_protocol::protocol::ListCustomPromptsResponseEvent;
    use rune_protocol::protocol::ListRemoteSkillsResponseEvent;
    use rune_protocol::protocol::ListSkillsResponseEvent;
    use rune_protocol::protocol::McpServerRefreshConfig;
    use rune_protocol::protocol::Op;
    use rune_protocol::protocol::RemoteSkillDownloadedEvent;
    use rune_protocol::protocol::RemoteSkillSummary;
    use rune_protocol::protocol::ReviewDecision;
    use rune_protocol::protocol::ReviewRequest;
    use rune_protocol::protocol::SkillsListEntry;
    use rune_protocol::protocol::ThreadNameUpdatedEvent;
    use rune_protocol::protocol::ThreadRolledBackEvent;
    use rune_protocol::protocol::TurnAbortReason;
    use rune_protocol::protocol::WarningEvent;
    use rune_protocol::request_user_input::RequestUserInputResponse;
    ...
    pub async fn resolve_elicitation(
        ...
        decision: rune_protocol::approvals::ElicitationAction,
    ) {
        let action = match decision {
            rune_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
            rune_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
            rune_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
        };
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| rune_protocol::message_history::HistoryEntry {
    ...
    ///
    /// This appends the name to `RUNE_HOME/sessions_index.jsonl` via `session_index::append_thread_name` for the
    ...
    pub async fn set_thread_name(sess: &Arc<Session>, sub_id: String, name: String) {
        ...
        let rune_home = sess.rune_home().await;
        if let Err(e) =
            session_index::append_thread_name(&rune_home, sess.conversation_id, &name).await
        {
            let event = Event {
                id: sub_id,
                msg: EventMsg::Error(ErrorEvent {
    ...
    pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
        ...
        sess.services
            ...
            .await;
        info!("Shutting down Rune instance");
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        final_output_json_schema: None,
        rune_linux_sandbox_exe: parent_turn_context.rune_linux_sandbox_exe.clone(),
        tool_call_gate: Arc::new(ReadinessFlag::new()),
...
pub(crate) async fn run_turn(
    ...
    // Although from the perspective of rune.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        if !pending_response_items.is_empty() {
            for response_item in pending_response_items {
                if let Some(TurnItem::UserMessage(user_message)) = parse_turn_item(&response_item) {
                    // todo(aibrahim): move pending input to be UserInput only to keep TextElements. context: https://github.com/openai/rune/pull/10656#discussion_r2765522480
        ...
        .await
        {
            ...
            Err(RuneErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(RuneErr::InvalidImageRequest()) => {
                let mut state = sess.state.lock().await;
                error_or_panic(
...
async fn run_auto_compact(sess: &Arc<Session>, turn_context: &Arc<TurnContext>) -> RuneResult<()> {
...
fn filter_rune_apps_mcp_tools(
    ...
    mcp_tools.retain(|_, tool| {
        ...
        let Some(connector_id) = rune_apps_connector_id(tool) else {
            return false;
        };
...
fn rune_apps_connector_id(tool: &crate::mcp_connection_manager::ToolInfo) -> Option<&str> {
...
async fn run_sampling_request(
    ...
    cancellation_token: CancellationToken,
) -> RuneResult<SamplingRequestResult> {
    ...
    if let Some(connectors) = connectors_for_tools.as_ref() {
        mcp_tools = filter_rune_apps_mcp_tools(mcp_tools, connectors);
    }
    ...
    loop {
        ...
        .await
        {
            ...
            Err(RuneErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(RuneErr::ContextWindowExceeded);
            }
            Err(RuneErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, rate_limits).await;
                }
                return Err(RuneErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                RuneErr::Stream(_, requested_delay) => {
...
fn agent_message_text(item: &rune_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: rune_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(rune_protocol::items::AgentMessageItem {
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, RuneResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> RuneResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut in_flight: FuturesOrdered<BoxFuture<'static, RuneResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: RuneResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(rune_async_utils::CancelErr::Cancelled) => break Err(RuneErr::TurnAborted),
        };

        let event = match event {
            Some(res) => res?,
            None => {
                break Err(RuneErr::Stream(
...
mod tests {
    ...
    use rune_protocol::mcp::CallToolResult as McpCallToolResult;
    ...
    async fn thread_rollback_fails_when_turn_in_progress() {
        ...
        *sess.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
        ...
        assert_eq!(
            error_event.rune_error_info,
            Some(RuneErrorInfo::ThreadRollbackFailed)
        );
    ...
    async fn set_rate_limits_retains_previous_credits() {
        let rune_home = tempfile::tempdir().expect("create temp dir");
        let config = build_test_config(rune_home.path()).await;
        let config = Arc::new(config);
        ...
        let initial = RateLimitSnapshot {
            ...
            plan_type: Some(rune_protocol::account::PlanType::Plus),
        };
    ...
    async fn set_rate_limits_updates_plan_type_when_present() {
        ...
        let update = RateLimitSnapshot {
            ...
            secondary: None,
            credits: None,
            plan_type: Some(rune_protocol::account::PlanType::Pro),
        };
        state.set_rate_limits(update.clone());
        ...
        loop {
            ...
            match evt.msg {
                EventMsg::Error(payload)
                    if payload.rune_error_info == Some(RuneErrorInfo::ThreadRollbackFailed) =>
                {
                    return payload;
                }
                _ => continue,
    ...
    async fn build_test_config(rune_home: &Path) -> Config {
    ...
    pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
        ...
        let skills_manager = Arc::new(SkillsManager::new(config.rune_home.clone()));
