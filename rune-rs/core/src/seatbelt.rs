...
pub async fn spawn_command_under_seatbelt(
    ...
    let arg0 = None;
    env.insert(RUNE_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
    spawn_child_async(
...
mod tests {
    ...
    #[test]
    fn create_seatbelt_args_with_read_only_git_and_rune_subpaths() {
        // Create a temporary workspace with two writable roots: one containing
        // top-level .git and .rune directories and one without them.
        ...
        // write to .rune/config.toml in the vulnerable root.
        ...
        // - write access to WRITABLE_ROOT_0 (but not its .git or .rune), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
"#,
        ...
        let mut expected_args = vec![
            ...
            format!(
                ...
                dot_rune_canonical.to_string_lossy()
            ),
            format!(
                "-DWRITABLE_ROOT_1={}",
        ...
        // Verify that .rune/config.toml cannot be modified under the generated
        // Seatbelt policy.
        let config_toml = dot_rune_canonical.join("config.toml");
        ...
        // Verify that writing a file to the folder containing .git and .rune is allowed.
    ...
    fn create_seatbelt_args_for_cwd_as_git_repo() {
        ...
        // `.rune` checks are done properly for cwd.
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: false,
...
"#,
    ...
    struct PopulatedTmp {
        /// Path containing a .git and .rune subfolder.
        ...
        /// ran `git commit` themselves, or modified .rune/config.toml to
        ...
        dot_rune_canonical: PathBuf,

        /// Path without .git or .rune subfolders.
        empty_root: PathBuf,
    ...
    fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
        ...
        fs::create_dir_all(vulnerable_root.join(".rune")).expect("create .rune");
        fs::write(
            vulnerable_root.join(".rune").join("config.toml"),
            "sandbox_mode = \"read-only\"\n",
        )
        .expect("write .rune/config.toml");
        ...
        let dot_rune_canonical = vulnerable_root_canonical.join(".rune");
