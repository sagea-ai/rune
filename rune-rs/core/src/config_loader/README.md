...
This module is the canonical place to **load and describe Rune configuration layers** (user config, CLI/session overrides, managed config, and MDM-managed preferences) and to produce:
...
Exported from `rune_core::config_loader`:

- `load_config_layers_state(rune_home, cwd_opt, cli_overrides, overrides, cloud_requirements) -> ConfigLayerStack`
- `ConfigLayerStack`
...
```rust
use rune_core::config_loader::{load_config_layers_state, LoaderOverrides};
