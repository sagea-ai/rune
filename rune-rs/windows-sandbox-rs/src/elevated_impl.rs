mod windows_impl {
    ...
    /// Creates the sandbox user's Rune home directory if it does not already exist.
    ...
    struct RunnerPayload {
        ...
        // Writable log dir for sandbox user (.rune in sandbox profile).
        rune_home: PathBuf,
    ...
    pub fn run_windows_sandbox_capture(
        ...
        ensure_rune_home_exists(&sandbox_base)?;

        let logs_base_dir: Option<&Path> = Some(sandbox_base.as_path());
        log_start(&command, logs_base_dir);
        let sandbox_creds =
            require_logon_sandbox_creds(&policy, sandbox_policy_cwd, cwd, &env_map, rune_home)?;
        ...
        let (psid_to_use, cap_sids) = match &policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => (
                ...
                vec![
                    ...
                    crate::cap::workspace_cap_sid_for_cwd(rune_home, cwd)?,
                ],
            ),
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        let payload = RunnerPayload {
            ...
            rune_home: sandbox_base.clone(),
            real_rune_home: rune_home.to_path_buf(),
