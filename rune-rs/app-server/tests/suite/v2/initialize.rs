...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    assert!(user_agent.starts_with("rune_vscode/"));
    Ok(())
}
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        rune_home.path(),
        &[(
            "RUNE_INTERNAL_ORIGINATOR_OVERRIDE",
            Some("rune_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("rune_originator_via_env_var/"));
    Ok(())
}
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        rune_home.path(),
        &[("RUNE_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
