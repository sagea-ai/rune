...
async fn test_rune_jsonrpc_conversation_flow() -> Result<()> {
    ...
    let send_user_id = mcp
        .send_send_user_message_request(SendUserMessageParams {
            conversation_id,
            items: vec![rune_app_server_protocol::InputItem::Text {
                text: "text".to_string(),
    ...
    let task_started_notification: JSONRPCNotification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("rune/event/task_started"),
    )
    .await??;
...
async fn test_send_user_turn_updates_sandbox_and_cwd_between_turns() -> Result<()> {
    ...
    let exec_begin_notification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("rune/event/exec_command_begin"),
    )
    .await??;
