use clap::Parser;
use clap::ValueHint;
use rune_common::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `rune resume` subcommand.
    // These are not exposed as user flags on the base `rune` command.
    #[clap(skip)]
    ...
    /// top-level `rune resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `rune fork` subcommand.
    // These are not exposed as user flags on the base `rune` command.
    #[clap(skip)]
    ...
    /// top-level `rune fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    pub sandbox_mode: Option<rune_common::SandboxModeCliArg>,
