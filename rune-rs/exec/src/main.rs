...
//! options and launches the non-interactive Rune agent. However, if it is
...
fn main() -> anyhow::Result<()> {
    arg0_dispatch_or_else(|rune_linux_sandbox_exe| async move {
        ...
        run_main(inner, rune_linux_sandbox_exe).await?;
        Ok(())
    })
}
...
mod tests {
    ...
    fn top_cli_parses_resume_prompt_after_config_flag() {
        ...
        let Some(rune_exec::Command::Resume(args)) = cli.inner.command else {
