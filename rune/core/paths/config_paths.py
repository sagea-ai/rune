...
from rune.core.paths.global_paths import RUNE_HOME, GlobalPath
from rune.core.trusted_folders import trusted_folders_manager
...
def _resolve_config_path(basename: str, type: Literal["file", "dir"]) -> Path:
    ...
    if not is_folder_trusted:
        return RUNE_HOME.path / basename
    if type == "file":
        if (candidate := cwd / ".rune" / basename).is_file():
            return candidate
    elif type == "dir":
        if (candidate := cwd / ".rune" / basename).is_dir():
            return candidate
    return RUNE_HOME.path / basename
...
def resolve_local_tools_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".rune" / "tools").is_dir():
        return candidate
    return None
...
def resolve_local_skills_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".rune" / "skills").is_dir():
        return candidate
    return None
...
def resolve_local_agents_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".rune" / "agents").is_dir():
        return candidate
    return None
...
HISTORY_FILE = ConfigPath(lambda: _resolve_config_path("runehistory", "file"))
