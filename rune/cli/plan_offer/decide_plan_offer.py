...
from rune.cli.plan_offer.ports.whoami_gateway import (
    WhoAmIGateway,
    WhoAmIGatewayError,
    WhoAmIGatewayUnauthorized,
    WhoAmIResponse,
)
from rune.core.config import DEFAULT_RUNE_API_ENV_KEY, Backend, ProviderConfig
...
def resolve_api_key_for_plan(provider: ProviderConfig) -> str | None:
    api_env_key = DEFAULT_RUNE_API_ENV_KEY

    if provider.backend == Backend.RUNE:
