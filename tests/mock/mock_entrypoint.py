...
Mocked returns are stored in the RUNE_MOCK_LLM_DATA environment variable.
"""
...
if __name__ == "__main__":
    ...
    from rune.core.types import LLMChunk
    ...
    patch(
        "rune.core.llm.backend.rune.RuneBackend.complete",
        side_effect=mock_complete,
    ).start()
    patch(
        "rune.core.llm.backend.generic.GenericBackend.complete",
        side_effect=mock_complete,
    ).start()
    patch(
        "rune.core.llm.backend.rune.RuneBackend.complete_streaming",
        side_effect=mock_complete_streaming,
    ).start()
    patch(
        "rune.core.llm.backend.generic.GenericBackend.complete_streaming",
        side_effect=mock_complete_streaming,
    ).start()

    from rune.acp.entrypoint import main
