...
from rune.core.config import load_dotenv_values
...
def test_sets_and_overrides_values(tmp_path: Path) -> None:
    ...
    assert environ["RUNE_API_KEY"] == "new-key"
...
def test_ignores_empty_values(tmp_path: Path) -> None:
    env_path = tmp_path / ".env"
    _write_env_file(
        env_path, "\n".join(["EMPTY=", "RUNE_API_KEY=", "NO_VALUE"]) + "\n"
    )
    ...
    assert "RUNE_API_KEY" not in environ
