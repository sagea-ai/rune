...
from rune.core.teleport.errors import (
    ServiceTeleportError,
    ServiceTeleportNotSupportedError,
)
from rune.core.teleport.git import GitRepoInfo
from rune.core.teleport.nuage import TeleportSession
from rune.core.teleport.teleport import _NUAGE_EXECUTION_URL_TEMPLATE, TeleportService
...
def _reimport_agent_loop() -> Any:
    to_clear = ("rune.core.agent_loop", "git", "rune.core.teleport")
    for k in [k for k in sys.modules if any(k.startswith(m) for m in to_clear)]:
        del sys.modules[k]
    return importlib.import_module("rune.core.agent_loop")
...
class TestTeleportServiceValidateConfig:
    def test_raises_when_no_api_key(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(
            ServiceTeleportError, match="STAGING_RUNE_API_KEY not set"
        ):
