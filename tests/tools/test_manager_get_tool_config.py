...
def test_merges_user_overrides_with_defaults():
    rune_config = build_test_rune_config(
        system_prompt_id="tests",
        include_project_context=False,
        tools={"bash": BaseToolConfig(permission=ToolPermission.ALWAYS)},
    )
    manager = ToolManager(lambda: rune_config)
...
def test_preserves_tool_specific_fields_from_overrides():
    ...
    rune_config.tools["bash"].__pydantic_extra__ = {"default_timeout": 600}
    manager = ToolManager(lambda: rune_config)
...
from rune.core.tools.base import BaseTool, BaseToolConfig, BaseToolState
...
class TestToolManagerModuleReuse:
    ...
    def test_multiple_managers_share_tool_classes(self):
        ...
        manager1 = ToolManager(lambda: rune_config)
        manager2 = ToolManager(lambda: rune_config)
