...
def acp_agent_loop(backend) -> RuneAcpAgentLoop:
    config = build_test_rune_config(
        ...
        models=[
            ModelConfig(
                name="devstral-latest", provider="rune", alias="devstral-latest"
            ),
            ModelConfig(
                name="devstral-small", provider="rune", alias="devstral-small"
            ),
        ],
    )
    ...
    patch("rune.acp.acp_agent_loop.AgentLoop", side_effect=PatchedAgentLoop).start()
