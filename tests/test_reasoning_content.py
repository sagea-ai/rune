...
class TestRuneMapperParseContent:
    ...
    def test_parse_content_text_chunk_returns_content_only(self):
        mapper = RuneMapper()
        content: list[runeai.ContentChunk] = [
            runeai.TextChunk(type="text", text="Hello from text chunk")
        ]
    ...
    def test_parse_content_thinking_chunk_extracts_reasoning(self):
        ...
        content: list[runeai.ContentChunk] = [
            runeai.ThinkChunk(
                type="thinking",
                thinking=[runeai.TextChunk(type="text", text="Let me think...")],
            ),
            runeai.TextChunk(type="text", text="The answer is 42."),
        ]
    ...
    def test_parse_content_multiple_thinking_chunks_concatenates(self):
        ...
        content: list[runeai.ContentChunk] = [
            runeai.ThinkChunk(
                type="thinking",
                thinking=[runeai.TextChunk(type="text", text="First thought. ")],
            ),
            runeai.ThinkChunk(
                type="thinking",
                thinking=[runeai.TextChunk(type="text", text="Second thought.")],
            ),
            runeai.TextChunk(type="text", text="Final answer."),
        ]
    ...
    def test_parse_content_thinking_only_returns_empty_content(self):
        ...
        content: list[runeai.ContentChunk] = [
            runeai.ThinkChunk(
                type="thinking",
                thinking=[runeai.TextChunk(type="text", text="Just thinking...")],
            )
        ]
...
class TestRuneMapperPrepareMessage:
    def test_prepare_assistant_message_without_reasoning(self):
        ...
        assert isinstance(result, runeai.AssistantMessage)
...
class TestRuneReasoningFieldNameValidation:
    def test_rune_backend_rejects_custom_reasoning_field_name(self):
        provider = ProviderConfig(
            name="rune",
            api_base="https://api.rune.ai/v1",
            api_key_env_var="RUNE_API_KEY",
            reasoning_field_name="reasoning",
        )

        with pytest.raises(ValueError) as exc_info:
            RuneBackend(provider=provider)

        assert "does not support custom reasoning_field_name" in str(exc_info.value)
        assert "reasoning" in str(exc_info.value)

    def test_rune_backend_accepts_default_reasoning_field_name(self):
        provider = ProviderConfig(
            name="rune",
            api_base="https://api.rune.ai/v1",
            api_key_env_var="RUNE_API_KEY",
        ...
        backend = RuneBackend(provider=provider)
