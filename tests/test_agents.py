...
from rune.core.agents.models import (
    BUILTIN_AGENTS,
    PLAN_AGENT_TOOLS,
    AgentProfile,
    AgentSafety,
...
class TestAgentApplyToConfig:
    def test_custom_prompt_found_in_global_when_missing_from_project(
        ...
        """Regression test for https://github.com/runeai/rune-cli/issues/288
        ...
        ~/.rune/prompts/ directory and load the file from there.
        """
        project_prompts = tmp_path / "project" / ".rune" / "prompts"
        project_prompts.mkdir(parents=True)

        global_prompts = tmp_path / "home" / ".rune" / "prompts"
        global_prompts.mkdir(parents=True)
        (global_prompts / "cc.md").write_text("Global custom prompt")

        monkeypatch.setattr(
            "rune.core.config.PROMPTS_DIR", ConfigPath(lambda: project_prompts)
        )
        monkeypatch.setattr(
